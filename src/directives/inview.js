...
function getPlugin() {
  ...
  function throttle(handler, timeout) {
    ...
    if (!handler || typeof handler !== "function")
      throw new Error(
        "Throttle handler argument is not incorrect. Must be a function."
      );
    var timeoutTime = 0;
    return function(e) {
  ...
  function createInstance(Vue, options) {
    ...
    function scrollHandler(e) {
      var viewportTop = window.pageYOffset,
        ...
        scrollPercent = roundPercent(
          window.pageYOffset / (documentHeight - viewportHeight)
        );
      ...
      function getInType(i) {
        var rect = i.element.getBoundingClientRect(),
          ...
          percentInView =
            topIn || bottomIn
              ? ((bottomIn ? elementBottom : viewportBottom) -
                  (topIn ? elementTop : viewportTop)) /
                rect.height
              : 0,
          centerPercent =
            (elementTop - viewportTop + rect.height / 2) / viewportHeight,
      ...
      for (var id in items) {
        ...
        var eventType =
          (inViewChange && EventTypes.Enter) ||
          (outViewChange && EventTypes.Exit) ||
          EventTypes.Progress;
  ...
  return {
    install: function(Vue, options) {
      Vue.directive(
        "view",
        Vue.prototype.$isServer ? {} : createInstance(Vue, options)
      );
    }
  };
}
